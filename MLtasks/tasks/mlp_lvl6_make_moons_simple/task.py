...
TASK_ID = "mlp_lvl5_make_moons_simple"
...
def get_task_metadata() -> Dict:
    return {
        ...
        "algorithm": "Simple MLP on make_moons",
        "metric": "accuracy",
    } 
...
def make_dataloaders(batch_size: int = 64, seed: int = 42) -> Tuple[DataLoader, DataLoader]:
    X, y = make_moons(n_samples=800, noise=0.18, random_state=seed)
...
def train(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, device: torch.device, epochs: int = 20, lr: float = 1e-2) -> Dict:
...
def predict(model: nn.Module, data_loader: DataLoader, device: torch.device) -> Tuple[np.ndarray, np.ndarray]:
    model.eval()
    probs_all = []
    preds_all = []
    with torch.no_grad():
...
def save_artifacts(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, history: Dict, device: torch.device) -> Dict:
    ...
    return {"decision_boundary": boundary_path, "metrics_json": metrics_path}
...
def main() -> int:
    ...
    success = val_metrics["accuracy"] > 0.90 and val_metrics["f1"] > 0.90
